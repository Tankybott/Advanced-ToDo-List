...
import { clearInput, addClass, checkIfInputNotEmpty, checkCheckbox, clearCheckbox, checkDataAttribute, checkIfContainsClass, removeClass, setDataAttribute, getInnerText, setCurrsorOnLastLetter, makeClickableOnly, resetClickableOnly, firstLetterToCapital, setInnerText, getInputValue} from './globalFunctions.js';
...
const handleButtons= (event) => {
    ...
    if (checkIfContainsClass(targetedButton, itemBtnImportantCss)){
        handleImportantBtn(targetedTask, targetedButton, targetedText, targetedButtonBox);
    } else if (checkIfContainsClass(targetedButton, itemBtnDoneCss, )){
        handleDoneBtn(targetedTask, targetedButton, targetedText, targetedButtonBox);
...
const handleImportantBtn = (task, button, text, buttonBox) => {
    ...
    const removeDoneStatus = () => {
        const doneBtn = buttonBox.querySelector(`.${itemBtnImportantCss}`);//selecting important button insaide task which was clicked
...
const handleEditBtn = (task, targetedButton, list) => {
    ...
    const removeEditStatus = () => {
        ...
        setInnerText(targetedText, firstLetterToCapital(getInnerText(targetedText))); // capitalizing first letter of task 
